...
class GateSimulator:
    ...
    def run_and_rotate(self, phases: np.ndarray, dt: float, roatation_phase: float) -> float:
        psi01 = self.params.initial_psi01.copy()
        psi11 = self.params.initial_psi11.copy()
        for phi in phases:
            U01 = scipy.linalg.expm(-1j*self.hams.H01(phi)*dt)
...
def simulate_gate_F(B, best_atom_params, roatation_phase, stark1, stark2, lifetime, lifetime2):
    ...
    # set up (Omega_Rabi1, blockade_inf, blockade, r_lifetime, delta1, Stark1, Stark2, resolution, optimizer, Lifetime2)
    params = PulseParameters(Rabi, 0, B, lifetime, 0, stark1, stark2, resolution, 0, lifetime2)
    hams = Hamiltonians(params)
    fid_calc = FidelityCalculator()
    simulator = GateSimulator(params, hams, fid_calc)
    ansatz = CosineAnsatz()
    ...
    fid, gp =  simulator.run(best_phase, dt)
    infid = 1-fid
    return infid
